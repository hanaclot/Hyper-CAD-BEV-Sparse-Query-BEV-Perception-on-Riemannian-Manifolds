...
# Bistable reaction
def bistable(u, X, Y, theta=0.4, strength=2.0):
    return strength * u * (1.0-u) * (u-theta)

noise_levels = {"low":0.3, "moderate":0.6, "high":1.0}
all_results = {}
for nl_name, nl_val in noise_levels.items():
    ...
    pde_eucl = ReactionDiffusionPDE(M_flat, gamma=0.5, dt=0.01, max_iter=200)
    ...
    pde_riem = ReactionDiffusionPDE(M_flat, gamma=0.5, dt=0.01, max_iter=200)
...
log("="*70)
log("TABLE II: PDE Denoising Ablation (moderate noise, sigma=0.09, sp=1.8%)")
table2 = [
...
for name, M_i in [("moderate", M_mod), ("steep", M_stp)]:
    ...
    pde_i = ReactionDiffusionPDE(M_i, gamma=0.5, dt=0.01, max_iter=200)
...
log(f"  Flat: IoU={iou_r:.4f}, Geo={geo_r:.1f}cm, EdgeF1={edge_r:.3f}")
...
table6c=[
    ...
    {"Slope":"15 deg (Moderate)","Ours_IoU":f"{iou_m:.4f}","MonoBEV_IoU":"0.623","Ours_Geo":f"{geo_m:.1f}cm","MonoBEV_Geo":"19.8cm"},
    {"Slope":"25 deg (Steep)","Ours_IoU":f"{iou_s:.4f}","MonoBEV_IoU":"0.417","Ours_Geo":f"{geo_s:.1f}cm","MonoBEV_Geo":"50.0cm"},
]
