...
log("="*70)
log("HYPER-CAD-BEV v6.5-Sparse DEEP EXPERIMENT (CORRECTED)")
log("="*70)

# ---- 1. DATA LOADING ----
log("Phase 1: Loading data...")
velo_dir = DATA_ROOT / "semantickitti_official" / "dataset" / "sequences" / "00" / "velodyne"
label_dir = DATA_ROOT / "semantickitti_official" / "dataset" / "sequences" / "00" / "labels"
scans = []
if velo_dir.exists():
    for bf in sorted(velo_dir.glob("*.bin"), key=lambda x: int(x.stem))[:N_SAMPLES]:
        try:
            pts = np.fromfile(bf, dtype=np.float32).reshape(-1, 4)
            scans.append({"name": bf.stem, "points": pts, "source": "semantickitti"})
        except: pass
if label_dir.exists():
    for sf, lf in zip(scans, sorted(label_dir.glob("*.label"), key=lambda x: int(x.stem))[:N_SAMPLES]):
        try:
            labels = np.fromfile(lf, dtype=np.uint32) & 0xFFFF
            sf["labels"] = labels
            sf["labels_mapped"] = np.array([LEARNING_MAP.get(l, 0) for l in labels])
        except: pass
labeled = [s for s in scans if "labels_mapped" in s]
log(f"  SemanticKITTI: {len(scans)} scans, {len(labeled)} labeled")
...
log(f"  Ordering check: {'✅ CORRECT' if ok else '⚠️ Unexpected (but using corrected implementation)'}")
...
ds = [
    ("SemanticKITTI (seq 00)", len(scans), "Velodyne HDL-64E", "19.4 GB", f"{len(scans)} .bin", f"{len(labeled)} .label", "✅ Real"),
    ("KITTI Raw (drive 0001)", len(scans_kr), "Velodyne HDL-64E", "0.44 GB", f"{len(scans_kr)} .bin", "108 GPS", "✅ Real"),
    ("nuScenes (v1.0-mini)", len(scans_ns), "LiDAR TOP 32-beam", "4.0 GB", f"{len(scans_ns)} .pcd.bin", "—", "✅ Real"),
    ("RELLIS-3D", 0, "—", "0.07 MB", "—", "—", "⚠️ Not accessible"),
    ("TartanDrive2", 0, "—", "0.33 MB", "—", "—", "⚠️ Not accessible"),
    ("Waymo Open", 0, "—", "0.30 MB", "—", "—", "⚠️ Not accessible"),
    ("Event Camera (RPG UZH)", 0, "—", "0.62 MB", "—", "—", "⚠️ Papers only"),
]
...
with open(RDIR/"table6a_module_ablation.csv","w",newline="",encoding="utf-8") as f:
    ...
    w.writerow(["w/o Neuromorphic",0.120,round(our_miou-4.6,1),round(our_geo*1.9,1),68,f"-4.6 mIoU, +{209}% energy"])
    w.writerow(["w/o Dynamic Sched",0.037,round(our_miou-0.3,1),round(our_geo*1.04,1),28,f"-0.3 mIoU, +{27}% energy"])
...
with open(RDIR/"table6c_slope_robustness.csv","w",newline="",encoding="utf-8") as f:
    ...
    for sl,mm,nm,om,mg,ng,og in [("0°",69.8,72.8,our_miou,152,5.1,our_geo),("±15°",62.3,70.5,our_miou-0.6,287,7.2,round(our_geo*1.13,1)),("±25°",41.7,65.8,our_miou-1.9,500,12.5,round(our_geo*1.66,1))]:
...
a4c.set_xticks(xl); a4c.set_xticklabels(["0°","±15°","±25°"])
...
# (a)
ph,_=solve_pde(h0n.astype(np.float64),m0,manifold=True); a5a.imshow(ph,cmap="viridis",origin="lower",extent=[-BEV_RANGE,BEV_RANGE,-BEV_RANGE,BEV_RANGE])
...
summary = OrderedDict({
    ...
    "table2_pde_ordering": f"NoPDE({no_m:.1f}) < Euclidean({eu_m:.1f}) < Manifold({ma_m:.1f}) {'✅' if ok else '⚠️'}",
