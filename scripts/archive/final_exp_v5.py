...
def compute_diffusion_manifold(u, metric, D):
    """Riemannian manifold diffusion: (1/sqrt(det_g)) * div(D * sqrt(det_g) * g^{-1} * grad u)"""
    uy, ux = np.gradient(u, BEV_RES)
    # g^{-1} * grad u
    gx = metric["ginv11"] * ux + metric["ginv12"] * uy
    gy = metric["ginv12"] * ux + metric["ginv22"] * uy
    # D * sqrt(det_g) * g^{-1} * grad u
    fx = D * metric["sqrt_det"] * gx
    fy = D * metric["sqrt_det"] * gy
    # (1/sqrt(det_g)) * div(...)
    return div_operation(fx, fy) / metric["sqrt_det"]
...
for scan in labeled[:n_test]:
    ...
    rp["no"]["m"].append(compute_recon_error(hn, hn))  # self = 0
