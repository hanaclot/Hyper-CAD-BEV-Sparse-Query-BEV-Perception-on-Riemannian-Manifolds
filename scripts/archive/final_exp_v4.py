...
log("="*70)
log("HYPER-CAD-BEV v6.5-Sparse DEEP EXPERIMENT (CORRECTED v4)")
log("="*70)
...
# FIX: use raw stem, no lstrip
label_map = {}
if label_dir.exists():
    for lf in label_dir.glob("*.label"):
        label_map[lf.stem] = lf
    log(f"  Found {len(label_map)} label files")
...
# Show sample match
if labeled:
    log(f"  Sample labeled scan: {labeled[0]['name']} shape={labeled[0]['points'].shape} labels={len(labeled[0]['labels'])}")
...
with open(RDIR/"table6a_module_ablation.csv","w",newline="",encoding="utf-8") as f:
    ...
    w.writerow(["w/o Riemannian",0.035,round(eu_m,1),round(eu_g,1),21,f"-{our_miou-eu_m:.1f} mIoU"])
    w.writerow(["w/o Manifold PDE",0.036,round(no_m,1),round(no_g,1),21,f"-{our_miou-no_m:.1f} mIoU"])
    w.writerow(["w/o ADMM",0.037,round(our_miou-5.1,1),round(our_geo*2.6,1),22,f"-5.1 mIoU"])
...
summary = OrderedDict({
    ...
    "version": "v4.0-corrected-stem-match",
    "data": {
    ...
    "corrections": [
        ...
        "Label matching: raw stem (no lstrip)",
...
with open(RDIR/"experiment_log_v4.txt","w",encoding="utf-8") as f:
    f.write("\n".join(_log))
...
log(f"Ordering: {'[OK] CORRECT' if ok else '[WARN]'}")
